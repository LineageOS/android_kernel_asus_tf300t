...
static struct rt5640_init_reg init_list[] = {
	...
	{RT5640_HP_VOL		, 0x8c8c},//OUTMIX -> HPVOL
...
//*************************************************************************************************
//*************************************************************************************************
static ssize_t read_audio_codec_status_rt5642(struct device *dev,struct device_attribute *attr, char *buf)
{
       return sprintf(buf, "%d\n", audio_codec_status);
}
DEVICE_ATTR(audio_codec_status_rt5642, S_IRUGO, read_audio_codec_status_rt5642, NULL);
...
static int rt5640_get_gain(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int rt5640_set_gain(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	unsigned int project_info = 0;
...
static const struct snd_kcontrol_new rt5640_snd_controls[] = {
	...
	SOC_SINGLE_BOOL_EXT("Recording Gain", 0,
		rt5640_get_gain, rt5640_set_gain),
...
static int rt5640_probe(struct snd_soc_codec *codec)
{
	...
	ret = gpio_direction_output(TEGRA_GPIO_PBB6, 1) ;
	if (ret) {
	...
	ret = device_create_file(codec->dev, &dev_attr_audio_codec_status_rt5642);
	if (ret != 0) {
		dev_err(codec->dev,"Failed to create audio_codec_status sysfs files: %d\n", ret);
		return ret;
	}
	if(snd_soc_read(codec, RT5640_VENDOR_ID1) == 0x10EC)
